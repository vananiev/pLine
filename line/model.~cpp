   ...
   Z0_ss(0,0)
{
   ...
   u_gt = 1000;
   num_gt_wire = 0;
   is_Transpositioned = false;
   d.resize(num_wire);
   d[0] = Complex(2.0, 18.5);
   d[1] = Complex(3.5, 14.5);
   d[2] = Complex(-2.0, 14.5);
   d_gt.resize(num_gt_wire);
   if(num_gt_wire>0) d_gt[0] = Complex(0, 20.5);
...
void link::Describe_Fields(){
   ...
   fi.Name = "Число ГТ(заземленных)";
   fi.Type = "int";
   fi.ptr = (void*) &num_gt_wire;
   Fields.push_back(fi);

   fi.Name = "ГТ(заземл.)кординаты подвеса на опоре,м";
   fi.Type = "vector<complex>";
   fi.ptr = (void*) &d_gt;
   Fields.push_back(fi);
   ...
   fi.Name = "Масса алюм.части пров,кг/км";
   fi.Type = "Float";
   fi.ptr = (void*) &m_Al;
   Fields.push_back(fi);
   ...
   Fields.push_back(fi);
   не  играет роли т.к. провод круглый
   */
...
void link::calc_k(){
   ...
   for(int i=0; i<num_f; i++){
      ...
      Matrix<Complex> Zv = sqrt((*Z0[i]) * (*Y0[i]).inverse());//(*Z0[i]) * y.inverse() ;

      vector< Matrix<Complex> > _k;
      Matrix<Complex> yl = Length * y ;
...
void link::rise_wire_tempetarute(){
  ...
  for(int i=0; i<num_wire; i++){   
   ...
   // коэф. теплоотдачи  на 1 провод (1 провод в расщепленной фазе)
   Float K = K_k + K_l;

   Float dP_sun = e_intake * D * 1000 /* * sin(q_sun)==1*/ * P_sun; // на 1 км
   ...
   Float I1_2 = norm(*I1[0][i]);
   Float I2_2 = norm(*I2[0][i]);
   Float Imax_2;
   Float w_num_2 = phase_wire_num*phase_wire_num;
   for(int i_freq=1; i_freq < num_f; i_freq++){
      I1_2 += 0.5*norm(*I1[i_freq][i])/w_num_2;
      I2_2 += 0.5*norm(*I2[i_freq][i])/w_num_2;
      }
   Imax_2 = (I1_2>I2_2)?I1_2:I2_2;
   Float dP_elec = Imax_2 * R0t[i];
...
void link::calc_Z0_Y0()
{
   ...
   for(int i=0; i<num_f;i++){
      bool save = (i==1)?true:false;
   	Matrix<Complex> * phase = calc_to_phase( Z0[i], save );
      delete Z0[i]; Z0[i] = NULL;
      Z0[i] = phase;
   	}
   ...
   Matrix<Float> * ph = calc_to_phase( P0, false ); //
   delete P0; P0 = NULL;
   // высчитываем емкости
...
void link::calc_Z0(){
   ...
   // коэф. увеличения длинны провода по сравнению с длинной ЛЭП
   // удельное активное сопротивление 1км провода и линии не равны из-за наличия провеса проводов (1км линии > 1км провода)
   Float K_wire_Length = 1 + Sag*Sag*8/3/spanLength/spanLength;
   Float K_wire_gt_Length = 1 + Sag_gt*Sag_gt*8/3/spanLength/spanLength;
  ...
  for(int n=0; n<num_f;n++){
   ...
   Float GMReq;
   if(phase_wire_num<=0){
      err(_ERROR_, "line: число проводов в расщепленной фазе должно быть больше 0");
      GMReq = GMRc;
   }else if(phase_wire_num == 1)
      GMReq = GMRc;
   else
      GMReq = pow( phase_wire_num * GMRc * pow(phase_radius,phase_wire_num-1), (Float)1.0/phase_wire_num );
   	...
   	for(int j=0; j<num; j++)
      	if(i==j){
         	if(i<num_wire){ // провод фазы
            	...
            	Re = (K_wire_Length*R0t[i])/phase_wire_num + pow(M_PI,2)*(1e-4)*f - f/(911.812*Derc/imag(d_sag[i])/2.0);
            }else{ // провод ГТ
            	Im = 4*M_PI*(1e-4)*f* (u_gt*0.25/gt_phase_wire_num + log(Derc/GMReq_gt) ) + f/(911.812*Derc/imag(d_sag[i])/2.0);
            	Re = (K_wire_gt_Length*R0_gt)/gt_phase_wire_num + pow(M_PI,2)*(1e-4)*f  - f/(911.812*Derc/imag(d_sag[i])/2.0);
               }
...
//---------------------------------------------------------------------------
template<class T>
Matrix<T> * link::calc_to_phase( Matrix<T> * _Z, bool save_Z_gt  )
{
	Matrix<T> * Zph = new Matrix<T>(num_wire, num_wire);
   // заземленных ГТ нет
   if( (int)_Z->Rows() == num_wire && (int)_Z->Cols() == num_wire ){
      if(save_Z_gt){
         Z0_as = Matrix<Complex>(0,0);
         Z0_ss = Matrix<Complex>(0,0);
         }
      *Zph = *_Z;
      return Zph;
   	}
   ...
   *Zph = (*Z_aa) - (*Z_as) * Z_ss->inverse() * Z_as->transpose();
   ...
   if(save_Z_gt){
      /* Сохраним сопротивления с участием гросотросов, нужно для обратного вычисления Z_aa для определения температуры провода */
      Z0_as = (Matrix<Complex>)*Z_as;
      Z0_ss = (Matrix<Complex>)*Z_ss;
      }
   ...
   for(int i_freq=0; i_freq<num_f; i_freq++){
    if(calc_non_nominal_freq || i_freq==1){
      ...
      do{
         ...
         for(int j=0; j<num; j++) (*X)[j][i_freq] = current_X[j][0]; // на основании (*Х) расчитывается update_B()
