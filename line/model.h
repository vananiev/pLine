...
class link : public element{
   private:
      ...
      void calc_d_sag();
      template<class T> Matrix<T> * calc_to_phase( Matrix<T> * _Z, bool save_Z_gt ); // пересчет фазных сопротивлений, искл. уравнения ГТ
   ...
   public:
      ...
      //Float q_sun;                        // угол наклона солнечных лучей к оси провода (не  играет роли т.к. провод круглый)
      Float T_wire[3];					      // температура провода фазы со стороны начала и конца
      Float T_env;                        // температура среды
      Float v, q;                         // скорость и угол атаки (рад) ветра
      Float m_Al;                         // масса алюминиевой части
      ...
      Matrix<Complex> Z0_as, Z0_ss;       // собственные и взаимные сопротивления для проводов ГТ, вычисленные для основной гармоники
      virtual string ClassName();			// имя класса
